    ...
    extends Agent
{
    ...
    protected boolean m_knowRumour2 = false;
    protected boolean m_knowRumour3 = false;
     ...
     */
    protected void passRumour( AID agent, String rumourString ) {
        if (m_knowRumour) {
            ACLMessage m = new ACLMessage( ACLMessage.INFORM );
        ...
        if (m_knowRumour2) {
            ACLMessage m = new ACLMessage( ACLMessage.INFORM );
            m.setContent( HostAgent.RUMOUR2 );
            m.addReceiver( agent );
            send( m );
        }
        ...
        if (m_knowRumour3) {
            ACLMessage m = new ACLMessage( ACLMessage.INFORM );
            m.setContent( HostAgent.RUMOUR3 );
            m.addReceiver( agent );
            send( m );
        }
    }
     ...
     */
    protected void hearRumour(String rumourString) {
    	if (HostAgent.RUMOUR.equals(rumourString)) {
    	...
    	} else if (HostAgent.RUMOUR2.equals(rumourString)) {
            // if I hear the rumour for the first time, tell the host
            if (!m_knowRumour2) {
                ACLMessage m = new ACLMessage( ACLMessage.INFORM );
                m.setContent( HostAgent.RUMOUR2 );
    	...
    	} else if (HostAgent.RUMOUR3.equals(rumourString)) {
            // if I hear the rumour for the first time, tell the host
            if (!m_knowRumour3) {
                ACLMessage m = new ACLMessage( ACLMessage.INFORM );
                m.setContent( HostAgent.RUMOUR3 );
