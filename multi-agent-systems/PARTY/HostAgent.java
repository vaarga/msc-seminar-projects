    ...
    extends Agent
{
    ...
    protected int m_rumour2Count = 0;
    protected int m_rumour3Count = 0;
            ...
            addBehaviour( new CyclicBehaviour( this ) {
                            public void action() {
                                ...
                                if (msg != null) {
                                    ...
                                    else if (RUMOUR.equals( msg.getContent() ) || RUMOUR2.equals( msg.getContent() ) || RUMOUR3.equals( msg.getContent() )) {
                                        // count the agents who have heard the rumour
                                        incrementRumourCount(msg.getContent());
                                    }
                                    else if (msg.getPerformative() == ACLMessage.REQUEST  &&  INTRODUCE.equals( msg.getContent() )) {
                                        // an agent has requested an introduction
                                        doIntroduction( msg.getSender(), msg.getContent() );
                                    }
                                }
                                else {
    ...
    protected void beginConversation() {
        ...
        doIntroduction( randomGuest( null ), RUMOUR );
        setPartyState( "Swinging" );
    }
    
    protected void beginConversation2() {
        // start a rumour
        ACLMessage rumour = new ACLMessage( ACLMessage.INFORM );
        rumour.setContent( RUMOUR2 );
        rumour.addReceiver( randomGuest( null ) );
        send( rumour );

        // introduce two agents to each other
        doIntroduction( randomGuest( null ), RUMOUR2 );
        setPartyState( "Swinging" );
    }

    protected void beginConversation3() {
        // start a rumour
        ACLMessage rumour = new ACLMessage( ACLMessage.INFORM );
        rumour.setContent( RUMOUR3 );
        rumour.addReceiver( randomGuest( null ) );
        send( rumour );

        // introduce two agents to each other
        doIntroduction( randomGuest( null ), RUMOUR3 );
        setPartyState( "Swinging" );
    }
     ...
     */
    protected void doIntroduction( AID guest0, String rumourString) {
        if (!m_partyOver) {
     ...
     */
    protected void incrementRumourCount(String rumourString) {    	
    	if (RUMOUR.equals(rumourString)) {
            m_rumourCount++;
            SwingUtilities.invokeLater( new Runnable() {
                                            public void run() {
            ...
            updateRumourAvg();
    	} else if (RUMOUR2.equals(rumourString)) {
            m_rumour2Count++;
            SwingUtilities.invokeLater( new Runnable() {
                                            public void run() {
                                                ((HostUIFrame) m_frame).prog_rumour2Count.setValue( Math.round( 100 * m_rumour2Count / m_guestCount ) );
                                            }
                                        } );
            updateRumour2Avg();
    	} else if (RUMOUR3.equals(rumourString)) {
            m_rumour3Count++;
            SwingUtilities.invokeLater( new Runnable() {
                                            public void run() {
                                                ((HostUIFrame) m_frame).prog_rumour3Count.setValue( Math.round( 100 * m_rumour3Count / m_guestCount ) );
                                            }
                                        } );
            updateRumour3Avg();
        }
        ...
        if (m_rumourCount == m_guestCount && m_rumour2Count == m_guestCount && m_rumour3Count == m_guestCount) {
    ...
    protected void updateRumour2Avg() {
        SwingUtilities.invokeLater( new Runnable() {
                                        public void run() {
                                            ((HostUIFrame) m_frame).lbl_rumour2Avg.setText( m_avgFormat.format( ((double) m_introductionCount) / m_rumour2Count ) );
                                        }
                                    } );
    }
    
    protected void updateRumour3Avg() {
        SwingUtilities.invokeLater( new Runnable() {
                                        public void run() {
                                            ((HostUIFrame) m_frame).lbl_rumour3Avg.setText( m_avgFormat.format( ((double) m_introductionCount) / m_rumour3Count ) );
                                        }
                                    } );
    }
